...
experiment_id = 'hubert_eval_CU_20231012'
...
#eval_lm = '/srv/scratch/z5313567/thesis/wav2vec2/model/AusKidTalk/progressive_finetune_AusTalk_CU_AusKidTalk_lowercase_freeze_base_model_20231003_with_lm_4gram_big'
...
if eval_pretrained:
    ...
    model = HubertForCTC.from_pretrained(eval_model, cache_dir = model_cache_fp)
    
    processor_LM = Wav2Vec2ProcessorWithLM.from_pretrained(eval_lm, cache_dir = model_cache_fp)
else:
    processor = Wav2Vec2Processor.from_pretrained(model_fp, cache_dir = model_cache_fp)
    model = HubertForCTC.from_pretrained(model_fp, cache_dir = model_cache_fp)
...
print("--> Getting fine-tuned alignment output with LM...")
word_output_with_LM = jiwer.process_words(results["target_text"], results["pred_str_with_LM"])
alignment_with_LM = jiwer.visualize_alignment(word_output_with_LM)
output_text_with_LM = "--> Getting the alignment result with LM...\n\n" + alignment_with_LM
output_text = output_text_without_LM + output_text_with_LM
...
if eval_baseline:
    ...
    model = HubertForCTC.from_pretrained(baseline_model, cache_dir=model_cache_fp)
    ...
    print("Baseline Test WER Without Language Model: {:.3f}".format(wer_metric.compute(predictions=results["pred_str_without_LM"], 
          references=results["target_text"])))
    print("Baseline Test WER With Language Model: {:.3f}".format(wer_metric.compute(predictions=results["pred_str_with_LM"], 
          references=results["target_text"])))
    print('\n')
    cer_metric = load_metric("cer")
    print("Baseline Test CER Without Language Model: {:.3f}".format(cer_metric.compute(predictions=results["pred_str_without_LM"], 
          references=results["target_text"])))
    print("Baseline Test CER With Language Model: {:.3f}".format(cer_metric.compute(predictions=results["pred_str_with_LM"], 
          references=results["target_text"])))
    print('\n')
    ...
    alignment_without_LM = jiwer.visualize_alignment(word_output_without_LM)
    output_text_without_LM = "--> Getting the alignment result without LM...\n\n" + alignment_without_LM + '\n\n\n'
    print("--> Getting baseline alignment output with LM...")
    word_output_with_LM = jiwer.process_words(results["target_text"], results["pred_str_with_LM"])
    alignment_with_LM = jiwer.visualize_alignment(word_output_with_LM)
    output_text_with_LM = "--> Getting the alignment result with LM...\n\n" + alignment_with_LM
    output_text = output_text_without_LM + output_text_with_LM
